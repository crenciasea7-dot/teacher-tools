...
export async function POST(request: Request) {
  ...
  if (!password || password !== process.env.ADMIN_PASSWORD) { const active = current && current.resetAt > now; attempts.set(ip, { count: (active ? current.count : 0) + 1, resetAt: active ? current.resetAt : now + 15 * 60 * 1000 }); return Response.json({ error: "비밀번호가 올바르지 않습니다." }, { status: 401 }); }
  attempts.delete(ip);
