...
const questions: Question[] = [
  ...
  { id: "cheap", label: "어떤 자산이 싸지?", lead: "절대가격이 아니라 다른 자산과 기회비용을 비교합니다.", tools: [
    { name: "Market Overview", description: "자산별 현재가와 등락률 확인", href: "/#macro" },
  ...
  { id: "buy", label: "사야 할까 말아야 할까?", lead: "살 수 있는지, 총비용을 감당할 수 있는지, 대안보다 나은지 확인합니다.", tools: [
    ...
    { name: "임장동선", description: "후보 단지의 방문 순서와 이동 동선 정리" },
    ...
    { name: "재개발투자금", description: "투자금·권리·단계별 위험 확인", href: "https://redevelopment-deal-analyzer.crenciasea7.chatgpt.site/", external: true },
    { name: "정부정책 영향", description: "대출·세금·시장심리에 미치는 영향 확인", href: "/research-insights" },
  ]},
...
export default function FlowGuide() {
  ...
  return <div className="flow-board">
    ...
    <section className="flow-result"><header><span>CURRENT QUESTION</span><h2>{active.label}</h2><p>{active.lead}</p></header><ol>{active.tools.map((tool, index) => <li key={tool.name}><i>{String(index + 1).padStart(2, "0")}</i><div><b>{tool.name}</b><span>{tool.description}</span></div>{tool.href ? <a href={tool.href} target={tool.external ? "_blank" : undefined} rel={tool.external ? "noreferrer" : undefined}>실행 →</a> : <em className="flow-soon">준비 중</em>}</li>)}</ol><div className="flow-external-checks"><a href="https://fin.land.naver.com/" target="_blank" rel="noreferrer">네이버 부동산 <small>외부 업체 ↗</small></a><a href="https://jaegebal.com/" target="_blank" rel="noreferrer">재개발닷컴 <small>외부 업체 ↗</small></a></div></section>
